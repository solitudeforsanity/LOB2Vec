...
def train_model_base(reason):
    build_representation = model.tcn_network()
    tcn_model = single_task_model(input_shape=(config.num_frames, config.h, config.w, config.d), network=build_representation, nb_classes=config.nb_st_classes)
    optimizer = Adam(lr = 0.00001)
    tcn_model.compile(loss=tf.keras.losses.CategoricalCrossentropy(), optimizer=optimizer,
    ...
    plot_model(build_representation, paths.model_images + '/TCN_CC_Model.png', show_shapes=True)
...
def model_fit(tcn_model, training_gen, validation_gen, model_name, steps_per_epoch_travelled, val_steps_per_epoch_travelled, reason, stock):
    ...
    csv_logger = CSVLogger(paths.logs_save + model_name + str(reason) + '_TCN_CC_Representation' + str(config.embedding_size), append=True, separator=';')
    logdir = paths.logs_save + model_name + str(reason) + '_TCN_CC_Representation' + str(config.embedding_size)
    ...
    evaluation.loss(trained_history, config.no_epochs, model_name + str(config.num_frames) + '_TCN_CC_' + stock)
    evaluation.acc(trained_history, config.no_epochs, model_name + str(config.num_frames) + '_TCN_CC_' + stock)
    return tcn_model
...
if __name__ == "__main__":
    ...
    for stock in config.stock_list:
        ...
        tcn_model = model_fit(tcn_model, training_gen, validation_gen, model_name, steps_per_epoch_travelled, val_steps_per_epoch_travelled, my_reason, stock)

        loss, acc, cp = tcn_model.evaluate(training_gen, verbose=2)
        print('Restored model, accuracy: {:5.2f}%'.format(100*acc))
        print('Restored model, accuracy: {:5.2f}%'.format(loss))
        pred = tcn_model.predict(testing_gen, steps=testing_gen.__len__())
        ...
        evaluation.plot_confusion_matrix(cnf_matrix, config.nb_st_classes, model_name + str(config.num_frames) + '_TCN_CC_' + stock)  # doctest: +SKIP
