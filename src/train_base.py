...
def train_model_base(X_train, Y_train, X_val, Y_val, labels_train, labels_val, num_frames, h, w, d, no_epochs, steps_per_epoch_travelled, batch_size, embedding_size, model_name, nb_classes, reason):
    ...
    trained_history = cc_model.fit(x=X_train, y=labels_train, batch_size=batch_size, epochs=no_epochs, verbose=2, callbacks=[early_stopper, csv_logger],
                                   validation_split=0.0, validation_data=[X_val, labels_val], shuffle=True, class_weight=None,
