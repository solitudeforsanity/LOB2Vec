...
def multi_task_model(input_shape, nb_classes, network, include_top=False, pooling=None):
    ...
    input = Input(shape=input_shape, name='input')
    encoded = network(input)
    y1 = Dense(nb_classes, activation='softmax', bias_initializer=model.initialize_bias, name='side')(encoded)
    ...
    plot_model(output, paths.model_images + '/TCN_CC_Multi_Task_Model.png', show_shapes=True)
    return output
...
def model_fit(tcn_model, training_gen, validation_gen, model_name, steps_per_epoch_travelled, val_steps_per_epoch_travelled, reason, stock):
    ...
    evaluation.multi_loss(trained_history, config.no_epochs, model_name + str(config.num_frames) + '_TCN_CC_' + stock)
    evaluation.multi_acc(trained_history, config.no_epochs, model_name + str(config.num_frames) + '_TCN_CC_' + stock)
    return tcn_model
...
if __name__ == "__main__":
    ...
    for stock in config.stock_list:
       ...
       # print('Restored model, accuracy: {:5.2f}%'.format(100*acc))
        ...
        print('Now Y Labels')
        y_labels = np.empty((0, config.nb_mt_classes))
        ...
        evaluation.plot_confusion_matrix(cnf_matrix, config.nb_st_classes, model_name + str(config.num_frames) + '_TCN_CC_MULTI_' + stock)  # doctest: +SKIP
